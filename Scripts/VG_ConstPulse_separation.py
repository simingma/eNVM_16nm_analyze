...
def IDSAT_separation(chip, col, L, Nfin, VT_flavor, VG, VD, Nrow, Nhci, Npbti, 
...
#        Imin = np.amin(np.array(bins)[:,0])
        ...
        mean_ = np.zeros(len(t_index))
        std_ = np.zeros(len(t_index))
        for idx in np.arange(len(t_index)):
            mean_[idx] = np.mean(1e6*IDSAT[hci_index, [t_index[idx]]*len(hci_index)])
            std_[idx] = np.std(1e6*IDSAT[hci_index, [t_index[idx]]*len(hci_index)])
        ...
        n_max = 0
        for (mean, std, t, label, axis) in zip(mean_, std_, t_index, time_label, ax):
            ...
            #axis.plot([0], [mean], 'r*', markersize=12)
            #axis.plot([0, 0], [mean-2*std, mean+2*std], 'y.', markersize=12)
            #axis.plot([0, 0], [mean-3*std, mean+3*std], 'y.', markersize=12)

            mean_lgd, = axis.plot([0], [mean], linestyle = 'None', color = 'r', marker = '*', markersize=12)
            sigma2, = axis.plot([0, 0], [mean-2*std, mean+2*std], linestyle = 'None', color = 'y', marker = '.', markersize=12)
            sigma3, = axis.plot([0, 0], [mean-3*std, mean+3*std], linestyle = 'None', color = 'g', marker = '.', markersize=12)
        ...
        plt.legend([mean_lgd, sigma2, sigma3], ['mean', '2$\sigma$', '3$\sigma$'], numpoints = 1)
