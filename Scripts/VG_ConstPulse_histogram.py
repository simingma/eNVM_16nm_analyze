...
def main():
    ...
    #I = np.sin(np.pi*t/12)
    f, ax = plt.subplots(nrows = 1, ncols = 6, sharey=True)
    print(len(ax))
    for k in np.arange(6):
        ##plt.subplot(1, 6, k+1)
        ##plt.plot(I, t+k)
        #ax[k].plot(I, t+k)
        ax[k].hist(I+k*0.2, orientation = 'horizontal')

    plt.subplots_adjust(wspace=0)
    #plt.setp([a.get_xticklabels() for a in f.axes[:]], visible=False)
    #plt.setp([a.get_yticklabels() for a in f.axes[1:]], visible=False)
    #ax[1].set_xticks([0.5], ['400ms'])
    #ax[2].set_xticks([0.5], ['recovery'])
    #ax[3].set_xticks([0.5], ['800ms'])
    #ax[4].set_xticks([0.5], ['recovery'])
    #ax[5].set_xticks([0.5], ['800ms'])
    plt.setp(ax[0], xticks=[], xticklabels=[])
    plt.setp(ax[1], xticks=[0], xticklabels=['400ms'])
    plt.setp(ax[2], xticks=[0], xticklabels=['recovery'])
    plt.setp(ax[3], xticks=[0], xticklabels=['800ms'])
    plt.setp(ax[4], xticks=[0], xticklabels=['recovery'])
    plt.setp(ax[5], xticks=[0, 1], xticklabels=['1200ms', 'recovery'])

    #plt.setp(ax[4], yticks=[], xticklabels=[])
    plt.show()
#    VD_values = 4 
