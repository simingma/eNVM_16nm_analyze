...
def main():
    ...
    """
# extract OFF_leakages from chip14's data
    chip = 14
    column = 6
    ...
    chip = 0
    column = 36
    path_data = '../data/VG_ConstPulse_chip00/'
    path_plot = '../plot/VG_ConstPulse_chip00_IDSAT/'
    #os.mkdir(path_plot)
...
#TODO: figure out the resolution/bits of the floating point current variable!!!
        ...
        Imin = 1e6*(np.amin(np.amin(IDSAT_VAsource_VBdrain), np.amin(IDSAT_VAdrain_VBsource)) - 8e-6)
        Imax = 1e6*(np.amax(np.amax(IDSAT_VAsource_VBdrain), np.amax(IDSAT_VAdrain_VBsource)) + 6e-6)
        ...
        figN = 1
        plt.figure(figN)
        plt.title('Col[8], L=20nm, 1fin, SVT, IDSAT (10 x 40ms Pulses) x 3 Cycles\nVGS=2V, 1st 400ms cycle VDS=1.3V, 2nd and 3rd 400ms cycles VDS=1.7V\nIDSAT measured at VGS=0.8, VDS=0.8, stress(forward) direction', fontsize=10)
